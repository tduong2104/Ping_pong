...
from ultralytics.nn.modules import Classify, C2f, Conv, Bottleneck
...
def train_v2(self: YOLO, pruning=False, **kwargs):
    ...
    self.task = "classify" #overrides.get('task') or self.task
    # print(Model.task_map)
    # print(Model.task_map["classify"])
    self.trainer = ClassificationTrainer(overrides=overrides, _callbacks=self.callbacks)
    # self.task_map[self.task]['trainer'](overrides=overrides, _callbacks=self.callbacks)
...
def prune(args):
    ...
    for i in range(args.iterative_steps):
        ...
        pruner = tp.pruner.MagnitudePruner(
            model.model,
            example_inputs,
            importance=tp.importance.MagnitudeImportance(p=1,group_reduction='mean'), #tp.importance.GroupNormImportance(),  # L2 norm pruning,
        ...
        print("Fine tune after pruning")
        model.train_v2(pruning=True, **pruning_cfg)

        # post fine-tuning validation
        pruning_cfg['name'] = f"val_after_finetune_pruned"
        pruning_cfg['batch'] = 1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data', default='flower_photos', help='Path to dataset')
    parser.add_argument('--epochs', default=5, type=int, help='Epoch to finetune')
    parser.add_argument('--prune_rate', default=0.5, type=float, help='Pruning ratio')
